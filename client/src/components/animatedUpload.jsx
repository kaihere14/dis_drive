...
export const FileUpload = forwardRef(({ onChange }, ref) => {
          ...
          <div className="relative w-full mt-6 max-w-xl mx-auto">
            {files.length > 0 &&
              files.slice(0, 2).map((file, idx) => (
                <motion.div
                  ...
                  className={cn(
                    "relative overflow-hidden z-40 bg-white flex flex-col items-start justify-start p-3 mt-3 w-full mx-auto rounded-md",
                    "shadow-sm",
                  )}
                >
            ...
            {files.length > 2 && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="mt-2 text-center text-sm text-neutral-500 font-medium"
              >
